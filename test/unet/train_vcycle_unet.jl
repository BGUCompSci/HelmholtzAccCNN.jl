...
function test_train_vcycle!(n, f, opt, lr, train_size, test_size, batch_size, iterations; data_augmentetion=false, e_vcycle_input=false, cifar_kappa=true, kappa_input=true, kappa_smooth=true, gamma_input=true, kernel=(3,3), v2_iter=10, level=3)
    ...
    random_batch = false
    resnet = false
    test_name = replace("3 small resnet=$("$(resnet)"[1]) kernel=$(kernel[1]) gamma=$("$(gamma_input)"[1]) da=$("$(data_augmentetion)"[1]) cifar=$("$(cifar_kappa)"[1]) smooth=1 e_vcycle=$("$(e_vcycle_input)"[1]) n=$(n) f=$(f) m=$(train_size) bs=$(batch_size) opt=$(SubString("$(opt)",1,findfirst("(", "$(opt)")[1]-1)) lr=$(lr) iter=$(iterations)","."=>"_")
