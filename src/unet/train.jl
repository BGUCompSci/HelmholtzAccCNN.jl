...
function residual_loss!(model, n, f, input, output; jac=false)
    r = reshape(input[:,:,1:2,1],n-1,n-1,2,1)|> cgpu
    kappa = reshape(input[:,:,3,1],n-1,n-1,1,1)|> cgpu
    gamma = reshape(input[:,:,4,1],n-1,n-1,1,1)|> cgpu
    ...
    if jac == true
        e0 = jacobi_channels!(n, e0, r./ (h^2), h_matrix)|> cgpu
    end
    r_unet = (h^2) .* helmholtz_chain_channels!(e0, h_matrix, n; h=h)|> cgpu

    return norm_diff!(r_unet, r)
end

function error_residual_loss!(model, n, f, input, output; w=0.1)
    e0 = model(input)
    ...
    return e_loss + w * r_loss
end

function error_residual_loss_details!(model, n, f, input, output; w=0.1)
    e0 = model(input)
    ...
    return [e_loss + w * r_loss, r_loss, e_loss]
end
...
function train_residual_unet!(model, test_name, n, f, kappa, omega, gamma,
    ...
    if data_path == "gmres"
        train_set = generate_gmres_data!(train_size, n, kappa, omega, gamma;
        ...
        test_set = generate_gmres_data!(test_size, n, kappa, omega, gamma;
    ...
    batchs = floor(Int64,train_size / batch_size)
    test_loss = zeros(iterations)
    train_loss = zeros(iterations)

    # Loss vs Epochs to csv file
    ...
    # Full solution loss
    full_loss!(x, y) = full_solution_loss_3!(model, x, y, n, f)
    full_loss_details!(tuple) = full_solution_loss_details_3!(model, tuple[1], tuple[2], n, f)

    # Error + residual loss
    r_loss!(x ,y) = error_residual_loss!(model, n, f, x, y)
    r_loss_details!(tuple) = error_residual_loss_details!(model, n, f, tuple[1], tuple[2])
    ...
    for iteration in 1:iterations
        ...
        if mod(iteration,smaller_lr) == 0
            ...
            batchs = floor(Int64,train_size / min((batch_size),train_size))
        ...
        for batch_idx in 1:batchs
            ...
            if full_loss == true
                Flux.train!(full_loss!, Flux.params(model), batch_set, opt)
            elseif residual_loss == true
                Flux.train!(r_loss!, Flux.params(model), batch_set, opt)
            else
...
function jacobi_channels!(n, x, b, matrix; w=0.8)
    ...
    y = helmholtz_chain_channels!(x, matrix, n; h=h)|> cgpu
    residual = b - y
...
function full_solution_loss!(model, input, output, n, f)
    r = reshape(input[:,:,1:2,1],n-1,n-1,2,1)|> cgpu
    kappa = reshape(input[:,:,3,1],n-1,n-1,1,1)|> cgpu
    gamma = reshape(input[:,:,4,1],n-1,n-1,1,1)|> cgpu
    omega = reshape([u_type(2.0*pi*f)],1,1,1,1)|> cgpu
    ...
    ae1 = (h^2) .* helmholtz_chain_channels!(e0, h_matrix, n; h=h)|> cgpu

    return norm_diff!(e0, e|> cgpu) + norm_diff!(ae1, r)
end

function full_solution_loss_details!(model, input, output, n, f)
    r = reshape(input[:,:,1:2,1],n-1,n-1,2,1)|> cgpu
    kappa = reshape(input[:,:,3,1],n-1,n-1,1,1)|> cgpu
    gamma = reshape(input[:,:,4,1],n-1,n-1,1,1)|> cgpu
    omega = reshape([u_type(2.0*pi*f)],1,1,1,1)|> cgpu
...
function full_solution_loss_3!(model, input, output, n, f)
    r = reshape(input[:,:,1:2,1],n-1,n-1,2,1)|> cgpu
    kappa = reshape(input[:,:,3,1],n-1,n-1,1,1)|> cgpu
    gamma = reshape(input[:,:,4,1],n-1,n-1,1,1)|> cgpu
    omega = reshape([u_type(2.0*pi*f)],1,1,1,1)|> cgpu
    ...
    ae1 = (h^2) .* helmholtz_chain_channels!(e1, h_matrix, n; h=h)|> cgpu
    r1 = r - ae1
...
function full_solution_loss_details_3!(model, input, output, n, f)
    r = reshape(input[:,:,1:2,1],n-1,n-1,2,1)|> cgpu
    kappa = reshape(input[:,:,3,1],n-1,n-1,1,1)|> cgpu
    gamma = reshape(input[:,:,4,1],n-1,n-1,1,1)|> cgpu
    omega = reshape([u_type(2.0*pi*f)],1,1,1,1)|> cgpu
    ...
    return [norm_diff!(e0, e|> cgpu) + norm_diff!(e3, e|> cgpu) + norm_diff!(e4, e|> cgpu),
            norm_diff!(e0, e|> cgpu), norm_diff!(e3, e|> cgpu), norm_diff!(e4, e|> cgpu)]
end
