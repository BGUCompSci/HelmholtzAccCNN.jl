...
function get_matrices!(alpha, kappa, omega, gamma)
    ...
    s_h_real = (kappa .* kappa .* omega .* omega) #.+ (kappa .* kappa .* omega .* gamma_i)
...
function jacobi_channels!(n, x, b, matrix; w=0.8)
    ...
    # @info "x $(size(x)) $(typeof(x)) matrix $(size(matrix)) $(typeof(matrix)) y $(size(y)) $(typeof(y)) b $(size(b)) $(typeof(b))"
    residual = b - y
    d = u_type(4.0 / h^2) .- sum(matrix, dims=4)
    # @info "d $(size(d)) $(typeof(d))"
    alpha = u_type(w) ./ d
    # @info "alpha $(size(alpha)) $(typeof(alpha))"
    step = alpha .* residual
    # @info "step $(size(step)) $(typeof(step))"
    #x = x + alpha .* residual
...
function full_solution_loss1!(model, input, output, n, f)
    r = reshape(input[:,:,1:2,1],n-1,n-1,2,1)|> cgpu
    kappa = reshape(input[:,:,3,1],n-1,n-1,1,1)|> cgpu
    gamma = reshape(input[:,:,4,1],n-1,n-1,1,1)|> cgpu
    omega = reshape([u_type(2.0*pi*f)],1,1,1,1)|> cgpu
...
function full_solution_loss_details1!(model, input, output, n, f)
    r = reshape(input[:,:,1:2,1],n-1,n-1,2,1)|> cgpu
    kappa = reshape(input[:,:,3,1],n-1,n-1,1,1)|> cgpu
    gamma = reshape(input[:,:,4,1],n-1,n-1,1,1)|> cgpu
    omega = reshape([u_type(2.0*pi*f)],1,1,1,1)|> cgpu
